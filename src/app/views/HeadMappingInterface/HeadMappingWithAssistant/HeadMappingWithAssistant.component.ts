...
export class OBPMetricBinding implements OnInit {
  ...
  GetAllMetricDetails(): void {
    this.PlanningrankingService.FetchObpMetricDetails(0).subscribe(
      (response) => {
  ...
  public pageSizes: (number | string)[] = [5, 10, 15, 20, 25];
  public selectedPageSize: number | string = 5;
  ...
  GetAllPlannerSessionWithType(): void {
    this.lpuPlannerServiceService
      .GetAllPlannerSessionWithType('P')
      .subscribe((response) => {
        if (response && response.item1 && response.item1.length > 0) {
          this.allPlannerSessions = response.item1;
          // If editing or selectedSessionId was already set, find session object
          if (this.selectedSessionId) {
            this.selectedPlannerSession =
              this.allPlannerSessions.find(
                (s) =>
                  s.id == this.selectedSessionId ||
                  (s as any).Id == this.selectedSessionId,
              ) || null;
          }
        } else {
  ...
  onPlannerSessionChange(event: any): void {
    ...
    if (val && val !== '' && val !== 'null' && val !== 'undefined') {
      this.selectedSessionId = parseInt(val, 10);
      this.selectedPlannerSession =
        this.allPlannerSessions.find(
          (s) =>
            s.id == this.selectedSessionId ||
  ...
  getToken(id: any) {
    this.authService.loginTemp(id).subscribe({
      next: (data) => {
        ...
        if (stMainElement) {
          stMainElement.innerHTML =
            'OBP Head<span class="themeClr"> Metric Mapping</span>';
        }

        const imgLogoElement = document.getElementById(
          'imgLogo',
        ) as HTMLInputElement;
        if (imgLogoElement) {
  ...
  public onMultiSelectChange(event: any): void {
    ...
    this.mappingForm
      .get('SchoolId')
      ?.setValue(this.selectedDivisions.length > 0 ? 'selected' : '');
  }
  ...
  public onEdit(record: MetricMapping | any): void {
    ...
    this.currentEditId = Number(
      get('Id') ?? get('id') ?? (record as any).Id ?? null,
    );

    if (!this.mappingForm.get('IsActive')) {
      this.mappingForm.addControl(
        'IsActive',
        this.fb.control('Yes', Validators.required),
      );
    } else {
    ...
    if (!this.mappingForm.get('Remarks')) {
      ...
      this.mappingForm.get('Remarks')?.setValidators([Validators.required]);
    }

    const headUid = get('HeadUID') ?? get('headUID') ?? get('headUid') ?? null;
    const assistantUid =
      get('AssistantUID') ?? get('assistantUID') ?? get('assistantUid') ?? null;
    const metricId =
      get('MetricId') ?? get('metricId') ?? get('metricid') ?? null;
    const rawSessionId =
      get('SessionId') ??
      get('sessionId') ??
      get('sessionID') ??
      get('PlannerSessionId') ??
      get('plannerSessionId') ??
      null;
    const rawStartDate = get('StartDate') ?? get('startDate') ?? null;
    const rawEndDate = get('EndDate') ?? get('endDate') ?? null;
    const rawRemarks = get('Remarks') ?? get('remarks') ?? '';

    this.AssignedToUid = headUid;
    this.AssistantUid = assistantUid;
    this.AssignToMetricId = metricId;

    const sessionIdVal = rawSessionId ? Number(rawSessionId) : null;
    this.selectedSessionId = sessionIdVal;
    this.selectedPlannerSession =
      this.allPlannerSessions.find((s) => s.id === sessionIdVal) || null;
    ...
    const rawSchoolDivisionIds =
      get('SchoolID') ??
      get('schoolID') ??
      get('SchoolDivisionIDs') ??
      get('SchoolIds') ??
      get('schoolIds') ??
      null;

    const schoolIds: any[] = rawSchoolDivisionIds
      ? String(rawSchoolDivisionIds)
          .split(',')
          .map((s) => s.trim())
          .filter((id) => id)
    ...
    this.mappingForm.patchValue({
      ...
      StartDate: this.formatDateForInput(rawStartDate),
      EndDate: this.formatDateForInput(rawEndDate),
    });

    this.mappingForm.get('IsActive')?.updateValueAndValidity();
    this.mappingForm.get('Remarks')?.updateValueAndValidity();
    this.mappingForm.get('SessionId')?.updateValueAndValidity();
  ...
  public onSubmit(): void {
    ...
    const formData = this.mappingForm.getRawValue();
    const schoolDivisionIDs = this.selectedDivisions.join(',');

    const headUid = formData.HeadUID ?? this.AssignedToUid ?? '';
    const assistantUid = formData.AssistantUID ?? this.AssistantUid ?? '';
    const metricId = formData.MetricId ?? this.AssignToMetricId ?? '';
    const typeVal = formData.Type ?? 'PA';
    const sessionIdVal = formData.SessionId ?? this.selectedSessionId ?? '';
    const startDateVal = formData.StartDate ? String(formData.StartDate) : '';
    const endDateVal = formData.EndDate ? String(formData.EndDate) : '';

    if (this.isUpdateMode && this.currentEditId !== null) {
      this.loadingIndicator = true;
      const MformData = new FormData();
      MformData.append('HeadUID', String(headUid));
      MformData.append('AssistantUID', String(assistantUid));
      MformData.append('MetricId', String(metricId));
      MformData.append('Type', String(typeVal));
      MformData.append('SchoolId', schoolDivisionIDs);
      MformData.append('StartDate', startDateVal);
      MformData.append('EndDate', endDateVal);
      MformData.append('SessionId', String(sessionIdVal));

      const remarksVal = String(this.mappingForm.get('Remarks')?.value ?? '');
      MformData.append('Remarks', remarksVal);

      const isActiveVal = this.normalizeIsActiveToYesNo(
        this.mappingForm.get('IsActive')?.value,
      );
      MformData.append('IsActive', isActiveVal);

      const idVal = String(this.currentEditId);
      MformData.append('Id', idVal);
      this.PlanningrankingService.updateRecord(MformData)
        .pipe(take(1))
        .subscribe({
          next: (res: any) => {
      ...
      this.PlanningrankingService.InsertHeadMapping(MformData)
        .pipe(take(1))
        .subscribe({
          next: (res: any) => {
            swal.fire({
    ...
    swal
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          this.PlanningrankingService.deleteRecord(MformData)
            .pipe(take(1))
            .subscribe({
              next: (res: any) => {
                const sres = res.item1[0];
                if (sres.msg === '-1') {
                  (swal.fire({ title: 'Action Failed', icon: 'error' }),
                    setTimeout(() => {
                      window.location.reload();
                    }, 112200));
                } else if (sres.msg === '1') {
  ...
  onHeadInput() {
    const inputValue = this.headControl.value
      ? String(this.headControl.value).toLowerCase().trim()
      : '';
    if (inputValue) {
      this.filteredHeadsData = this.EmployeeData.filter(
        (employee) =>
          employee.employeeName.toLowerCase().includes(inputValue) ||
          employee.employeeCode.toString().includes(inputValue),
      ).slice(0, 10);
      this.showHeadSuggestions = true;

      const codeMatch = inputValue.match(/\b\d+\b/);
      if (codeMatch) {
        const code = codeMatch[0];
        this.AssignedToUid = code;
        this.mappingForm.get('HeadUID')?.setValue(String(code));
      }
    } else {
      this.filteredHeadsData = [];
      this.showHeadSuggestions = false;
      this.AssignedToUid = null;
      this.mappingForm.get('HeadUID')?.setValue(null);
    }
  ...
  selectHead(employee: Employee) {
    this.AssignedToUid = employee.employeeCode;
    this.mappingForm.get('HeadUID')?.setValue(String(employee.employeeCode));
    this.headControl.setValue(
      `${employee.employeeName} (${employee.employeeCode})`,
    );
  ...
  onAssistantInput() {
    const inputValue = this.assistantControl.value
      ? String(this.assistantControl.value).toLowerCase().trim()
      : '';
    if (inputValue) {
      this.filteredAssistantsData = this.EmployeeData.filter(
        (employee) =>
          employee.employeeName.toLowerCase().includes(inputValue) ||
          employee.employeeCode.toString().includes(inputValue),
      ).slice(0, 10);
      ...
      if (codeMatch) {
        const code = codeMatch[0];
        this.AssistantUid = code;
        this.mappingForm.get('AssistantUID')?.setValue(String(code));
      }
    } else {
      this.filteredAssistantsData = [];
      this.showAssistantSuggestions = false;
      this.AssistantUid = null;
      this.mappingForm.get('AssistantUID')?.setValue(null);
    }
  ...
  selectAssistant(employee: Employee) {
    ...
    this.mappingForm
      .get('AssistantUID')
      ?.setValue(String(employee.employeeCode));
    this.assistantControl.setValue(
      `${employee.employeeName} (${employee.employeeCode})`,
    );
  ...
  onKeydown(event: KeyboardEvent, type: 'head' | 'assistant' | 'metric') {
    ...
    if (event.key === 'ArrowDown') {
      ...
      activeIndex =
        (activeIndex - 1 + filteredData.length) % filteredData.length;
  ...
  onMetricInput() {
    const inputValue = this.metricControl.value
      ? String(this.metricControl.value).toLowerCase().trim()
      : '';
    if (inputValue) {
      this.filteredMetricData = this.allMetricDescription
        .filter(
          (metric) =>
            metric.id.toString().includes(inputValue) ||
            (metric.description &&
              metric.description.toLowerCase().includes(inputValue)),
        )
        .slice(0, 15);
      this.showMetricSuggestions = true;

      // Auto-detect metric ID if typed directly or starting with numbers
      const matchLeading = inputValue.match(/^(\d+)/);
      if (matchLeading) {
        const parsedId = Number(matchLeading[1]);
        this.AssignToMetricId = parsedId;
        this.mappingForm.get('MetricId')?.setValue(String(parsedId));
      } else {
        const directMatch = this.allMetricDescription.find(
          (m) => m.description && m.description.toLowerCase() === inputValue,
        );
        if (directMatch) {
          this.AssignToMetricId = directMatch.id;
          this.mappingForm.get('MetricId')?.setValue(String(directMatch.id));
        }
      }
    } else {
      this.filteredMetricData = [];
      this.showMetricSuggestions = false;
      this.AssignToMetricId = null;
      this.mappingForm.get('MetricId')?.setValue(null);
    }
    this.activeMetricSuggestionIndex = -1;
  }
  selectMetric(metric: MetricDetails) {
    this.AssignToMetricId = metric.id;
    this.mappingForm.get('MetricId')?.setValue(String(metric.id));
    this.metricControl.setValue(`${metric.id} :: ${metric.description}`);
  ...
  getEmployeeDisplay(uid: string | number | null | undefined): string {
    ...
    const employee = this.EmployeeData.find(
      (e) => e.employeeCode.toString() === uid.toString(),
    );
    return employee
      ? `${employee.employeeName} (${employee.employeeCode})`
      : `${uid}`;
  ...
  getMetricDescriptionDisplay(id: string | number | null | undefined): string {
    if (!id) return '';
    const metric = this.allMetricDescription.find(
      (m) => m.id.toString() === id.toString(),
    );
    return metric ? `${metric.description} (${metric.id})` : `ID ${id}`;
  }

  getSchoolNamesDisplay(
    ids: (number | string)[] | string | null | undefined,
  ): string {
    if (!ids) return '';
