...
@Component({
  selector: 'app-placement-drive-hod-aaproval',
  templateUrl: './placement-drive-hodapproval.component.html',
  styleUrls: ['./placement-drive-hodapproval.component.scss'],
  changeDetection: ChangeDetectionStrategy.Default
})

export class PlacementDriveHODApprovalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    (<HTMLInputElement>document.getElementById('stMain')).innerHTML = 'Feedback of drive wise recruitment <span class="themeClr" >and selection process</span>';
...
debugger;
        this.storageService.saveUser(data);
         this.placementService.getAuthBatchyears('HOD').subscribe({
           next: data => {
...
getCompany(batchYear:any){
  ...
  this.streamData = [];
  this.placementService.getCompanyByAuthBatchyears(batchYear,'HOD').subscribe({
    next: data => {
...
getStream(companyId:any,batchYear:any){
  this.ngSelectComponentStream.handleClearClick();
  this.placementService.getStreamByAuthBatchyears(batchYear,companyId,'HOD').subscribe({
    next: data => {
...
getDriveStream(companyId:any,batchYear:any,stream:any){
  this.myArray=[];
  this.placementService.getAuthDriveByStreamBatchyears(batchYear,companyId,stream,'HOD').subscribe({
    next: data => {
     ...
     this.placementService.getSoftSkillData(checkData).subscribe({
      next: data => {
       ...
       // (<HTMLInputElement>document.getElementById('chk'+dExistData[pp]['driveId'])).style.display='block';
...
alfterTimerData(){
  ...
  let ddd = this.dExitDataAll.filter(x=>x['driveId']==this.myArray[i]['driveId'] && x['roundId']==this.myArray[i]['value']);
    ...
    (<HTMLInputElement>document.getElementById('CompanyRemarks'+this.myArray[i]['driveId']+this.myArray[i]['roundid'])).disabled = true;
    (<HTMLInputElement>document.getElementById('FacultyRemarks'+this.myArray[i]['driveId']+this.myArray[i]['roundid'])).disabled = true;
    (<HTMLInputElement>document.getElementById('Feedback'+this.myArray[i]['driveId']+this.myArray[i]['roundid'])).disabled = true;
