...
@Component({
  selector: 'app-planner-score-ranking',
  templateUrl: './planner-score-ranking.component.html',
  styleUrls: ['./planner-score-ranking.component.scss'],
  changeDetection: ChangeDetectionStrategy.Default
})

export class PlannerScoreRankingComponent implements OnInit {
  activeLink: string = 'LFTS';
  ...
  overallData: any=[];
  facultywiseData: any = [];
  selectedWingName:any='Overall';
  first:any;
  second:any;
  third:any;
  selectedSourceWingId:any='0';
  settoggle:string='0';
  ...
  constructor(private fb: FormBuilder,private cdRef: ChangeDetectorRef,
    ...
    private planningRankingService:PlanningrankingService,private spinner: NgxSpinnerService
  ) {
...
for(let i=0; i < els.length; i++){
  debugger;
  if(els[i].classList.contains('hideBlock'))
  {
         els[i].classList.remove('hideBlock');
        els[i].classList.add('showBlock');
        (<HTMLInputElement>document.getElementById('downimage')).style.display='none';
        (<HTMLInputElement>document.getElementById('upimage')).style.display='';
  }
  else{
    if(els[i].classList[3]=='permShow')
    {
      els[i].classList.remove('showBlock');
      els[i].classList.remove('hideBlock');
    }
    else{
    els[i].classList.remove('showBlock');
    els[i].classList.add('hideBlock');
  }
  (<HTMLInputElement>document.getElementById('downimage')).style.display='';
  (<HTMLInputElement>document.getElementById('upimage')).style.display='none';
  }
  debugger;
}
  ...
  getwingList(){
this.spinner.show();
    this.planningRankingService.getwinglist().subscribe({
      next: data => {    
       this.wingListData = data.item1;
       //console.log(JSON.stringify(this.wingListData));
       this.spinner.hide();
       this.getPlanningScoreRanking('0','0','Overall');    
  ...
  setwing(row:any){
    ...
    this.selectedSourceWingId=row.id;
    this.selectedWingName=row.wingName;
  }


  getPlanningScoreRanking(type:any,sourceWingId:any,wingname:any){
 //  alert('inside'+wingname.toString());
 this.spinner.show();
    const scoreData = {
      SourceDivision:sourceWingId.toString()==''? this.selectedSourceWingId.toString():sourceWingId.toString(),
      MetricIds:"0",
      PlannerSessionId:"9,11",
      IsVerified:"1",
      Type:"HOS",
      PlannerPart:"A",
      MetricType:"A,M",
     ...
     this.selectedWingName=wingname.toString()==''?this.selectedWingName.toString():wingname.toString();
    // alert('oo'+this.selectedWingName);
    this.planningRankingService.getplannerScoreRankingCalculation(scoreData).subscribe({
      next: data => {
        this.getPlanningScoreRanking1('1','0','Overall');
        if(type=="0"){
          this.overallData = data.item1;
          this.first=this.overallData[0];
          this.second=this.overallData[1];
          this.third=this.overallData[2];
        } 
        else{
          //this.facultywiseData="";
          this.facultywiseData = data.item1;
          if(type=='3'){
            this.facultywiseData.splice(2, 0, {
              name: 'NA',
              averageScore: 'NA',
              hosUID: 'NA'
          });
      ...
      //  console.log(JSON.stringify(this.facultywiseData));
      },
      });
  }


  getPlanningScoreRanking1(type:any,sourceWingId:any,wingname:any){
    //  alert('inside'+wingname.toString());
    this.spinner.show();
       const scoreData = {
         SourceDivision:sourceWingId.toString()==''? this.selectedSourceWingId.toString():sourceWingId.toString(),
        ...
        this.storageService.saveUser(data);
         this.placementService.getPlannerSession().subscribe({
           next: data => {
            ...
            this.getwingList();
            // this.getPlanningScoreRanking("0");
            // this.getPlanningScoreRanking("1");
