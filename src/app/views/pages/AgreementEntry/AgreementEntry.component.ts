...
@Component({
  selector: 'app-AgreementEntry',
  templateUrl: './AgreementEntry.component.html',
  styleUrls: ['./AgreementEntry.component.scss'],
...
commercialRows:any=1;
  ...
  isButtonShwoingNew:boolean=false;
  responses: any[]=[];
  results: RESULT={
    batchYear:0,
    companyId:0,
...
this.AgreementType2=[
  ...
  changeDivisionInvolved(event:any){
    
    let df = '';
  for(let i=0; i < event.length; i++){
    df += event[i]['id'] + ','
  }
  this.DivisionsInvolvedA = df;
  }
        ...
        this.storageService.saveUser(data);
         this.Agreement.getRunningSession().subscribe({
           next: data => {
            ...
            this.Agreement.getMouAgreementMasterCreatedby().subscribe({
              next: data => {
               
              this.TableDataCreatedBy = data.item1;
...
if($event != undefined){
  ...
  getMoUByloginName(loginName:any){
    ...
    this.Agreement.getMoUByloginName().subscribe({
      next: data => {
  ...
  changeSubType(event:any){
    ...
    this.selectedAgreementSubType = df;
...
this.isButtonShwoing = true;
  if(this.enterMouName===''){
    swal.fire(

      {title: 'MOU Agreement Approval', text: 'Please enter mou name !', icon: 'error'}

      );
      this.isButtonShwoing = false;
  }
  else  if(this.selectedAgreementType===''){
    swal.fire(

      {title: 'MOU Agreement Approval', text: 'Please select agreement type !', icon: 'error'}

      );
      this.isButtonShwoing = false;
  }
  else  if(this.selectedAgreementSubType==='' && this.AgreementTypeDisplay2.length > 0){
    swal.fire(

      {title: 'MOU Agreement Approval', text: 'Please select agreement sub type !', icon: 'error'}
    ...
    swal.fire(
      ...
      {title: 'MOU Agreement Approval', text: 'Please select mou agreement file !', icon: 'error'}
...
const denominations =
{
  ...
  commericialInvolved:this.roundDataBackUp
}
denominations.commericialInvolved = [];
...
if(this.isCommercialShowing === true){
  if( denominations.commericialInvolved.length === 0){
    swal.fire(
...
this.Agreement.addMouEntry(this.responses[0]).subscribe({
  next: data => {
    ...
    swal.fire(  {title: 'MOU Punching ', text: 'Mou Agreeement Punched Successfully  !', icon: 'success'}).then(function() {
      window.location.reload();
  });
