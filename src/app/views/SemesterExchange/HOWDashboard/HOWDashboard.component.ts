...
@Component({
  selector: 'app-HOWDashboard',
  templateUrl: './HOWDashboard.component.html',
  styleUrls: ['./HOWDashboard.component.css']
})
export class HOWDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.title.setTitle('Semester Exchange HOW Dashboard');
  ...
  getToken(loginName: string): void {
    ...
    (<HTMLInputElement>document.getElementById('stMain')).innerHTML = '<span class="themeClr"> HOW </span> Dashboard';
    ...
    this.ServicesSM.getAllApplications().subscribe({
      next: response => {
        if (Array.isArray(response.item1) && response.item1.length > 0) {
          this.AllApplications = response.item1;//.filter((item: { isForwardedtoHOW: boolean | null; }) => item.isForwardedtoHOW === true);
          // this.AllApplications = response.item1.filter((item: { isForwardedtoHOW: boolean | null; }) => item.isForwardedtoHOW === true);

          // Check if there are any applications after filtering
  ...
  //         this.AllApplications = response.item1.filter((item: { isForwardedtoHOW: boolean; }) => item.isForwardedtoHOW === true);
  //         // this.AllApplications = response.item1.filter((item: { isForwardedtoHOW: boolean; }) => item.isForwardedtoHOW === true);

  //         // console.log(JSON.stringify(response.item1.filter((item: { isForwardedtoHOW: boolean; }) => item.isForwardedtoHOW === null)))
