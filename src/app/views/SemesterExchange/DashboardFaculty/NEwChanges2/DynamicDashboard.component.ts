...
export class DynamicDashboardComponent implements OnInit {
   ...
   * Controls whether the Evaluation Marks section is shown:
   *   'counsellor' → hidden  (req #3/#4 — counsellor cannot see evaluation)
   *   'faculty' | 'hod' | 'how' → visible
   */
  selectedRemarksCallerRole: 'counsellor' | 'faculty' | 'hod' | 'how' = 'counsellor';

  /**
   * Holds existing faculty remarks text for the currently opened Add Remarks modal.
   * New text is APPENDED to this value, never overwrites it (req #1).
   */
  private existingFacultyRemarks = '';
  ...
  private buildPageTitle(): void {
    const roles: string[] = [];
    if (this.isHOD)              roles.push('HOD');
    if (this.isHoW)              roles.push('HoW');
    if (this.isdealingFaculty)   roles.push('Faculty');
    if (this.isDealingAuthority) roles.push('Counsellor');
  ...
  openAddRemarksModal(application: Application): void {
    ...
    // Capture existing faculty remarks so new input is APPENDED (req #1)
    const existing = this.AllAuthorityRemarks.find(
      r => r.registrationNo === application.registrationNo
    );
    this.existingFacultyRemarks =
      existing?.dealingUserInterviewRemarks || existing?.facultyRemarks || '';
  ...
  submitAddRemarksForm(): void {
    ...
    // Build appended remarks string — new entry is added after existing ones (req #1)
    const newText   = (this.AddRemarksForm.value.Comments as string).trim();
    const timestamp = new Date().toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' });
    const suffix    = `  [Added By: ${this.EmployeeCode} on ${timestamp}]`;
    const combined  = this.existingFacultyRemarks
      ? `${this.existingFacultyRemarks}\n---\n${newText}${suffix}`
      : `${newText}${suffix}`;
    ...
    fd.append('RegistrationNo',     this.RegistrationNo || '');
    fd.append('ApplicationId',      this.ApplicationId  || '');
    fd.append('CounsellingRemarks', combined);
    ...
    this.ServicesSM.UpdateCounsellingRemarks(fd).pipe(
      ...
      next: (data: any) => {
        const code = data?.item1?.[0]?.returnId;
        // code > 0 → new record created; code === -1 → record already exists (update)
        // Both are treated as success since we are intentionally appending
        if (code > 0 || code === -1) {
   ...
   *
   * @param row        Application row that was clicked
   * @param callerRole Role of the user clicking the button.
   *                   'counsellor' → Evaluation section is HIDDEN (req #3/#4)
