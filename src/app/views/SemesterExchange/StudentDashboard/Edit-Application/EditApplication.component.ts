...
//   templateUrl: './NewEdit-Application.html',
...
//   // ── Employee info ────────────────────────────────────────────────────────────
//   DealingFacultyName: any; 
//   CounsellingAuthorityName: any;
...
//       this.form.get('sponsorType')?.valueChanges.subscribe((value: string) => {
//         const sponsorType = (value || '').trim().toLowerCase();
...
//           this.form.get('sponsorRelation')?.clearValidators();
//           this.form.get('sponsorEmail')?.clearValidators();
//           this.form.get('sponsorContact')?.clearValidators();
//         } else {
...
//         this.form.get('sponsorName')?.updateValueAndValidity();
//         this.form.get('sponsorRelation')?.updateValueAndValidity();
//         this.form.get('sponsorEmail')?.updateValueAndValidity();
//         this.form.get('sponsorContact')?.updateValueAndValidity();
//       });
//     }
//   }

//   /** ── KEEP LOGIC: GET TOKEN ────────────────────────────────────────── */
...
//     if (el) {
//       (el as HTMLInputElement).innerHTML = 'Semester <span class="text-info">Exchange Student </span> Dashboard';
//     }
...
//     this.StagesDetail[index].fileObject = finalFile; 
...
//     if (step === 1) {
//       const requiredFields = ['EmailId', 'CountryName', 'WhatsAppNo', 'PhoneNumber', 'ParentContact', 'HasRelativeDetails'];
//       for (const field of requiredFields) {
//         const value = this.form.get(field)?.value;
//         if (value === null || value === undefined || value === '') return false;
//       }
//       if (this.form.get('HasRelativeDetails')?.value === 'Yes') {
//         if (!this.form.get('RelativeName')?.value || !this.form.get('RelativeRelation')?.value || !this.form.get('RelativeCountryName')?.value) {
//           return false;
//         }
//       }
//       return true;
...
//     if (step === 2) {
//       const requiredFields = ['ApplyingOption', 'UniversityOption1', 'UniversityOption2', 'UniversityOption3'];
//       for (const field of requiredFields) {
//         const value = this.form.get(field)?.value;
//         if (value === null || value === undefined || value === '') return false;
//       }
...
//     if (step === 3) {
//       const requiredFields = ['PassportStatus', 'IsVisaRejected', 'EnglishTestType', 'SponsorType', 'AvailableFunds'];
//       for (const field of requiredFields) {
//         const value = this.form.get(field)?.value;
//         if (value === null || value === undefined || value === '') return false;
//       }
...
//         if (!this.form.get('PassportNumber')?.value || !this.form.get('PassportIssueDate')?.value || !this.form.get('PassportValidUpto')?.value) {
//           return false;
//         }
//       }

//       if (this.form.get('IsVisaRejected')?.value === 'Yes') {
//         if (!this.form.get('VisaRejectedReason')?.value || !this.form.get('VisaRejectedCountry')?.value) {
//           return false;
//         }
//       }
...
//         if (!this.form.get('TestName')?.value || !year || !/^\d{4}$/.test(String(year))) {
//           return false;
//         }
//       }

//       if (this.form.get('EnglishTestType')?.value === 'Applied') {
//         if (!this.form.get('TestName')?.value) return false;
//       }
//       return true;
//     }
//     return true;
...
//   isParentSponsor(): boolean {
//     const sponsorType = this.form.get('sponsorType')?.value;
//     const sponsorName = this.form.get('sponsorName')?.value;
//     return ['Parent', 'Parents', 'parent','parents'].includes((sponsorType || '' || sponsorName).trim());
//   }
...
//   /** ── KEEP LOGIC: GET APPLICATION DETAILS ──────────────────────────── */
...
//           this.LockedStatus = this.stuApplication.isLocked === 'True';
//           this.DealingFacultyName = this.stuApplication.dealingFacultyName;
//           this.CounsellingAuthorityName = this.stuApplication.counsellingAuthorityName;
...
//             PassportStatus: this.stuApplication.passportStatus || 'No', 
...
//             IsVisaRejected: this.stuApplication.isVisaRejected === 'Yes' ? 'Yes' : 'No', 
//             VisaRejectedReason: this.stuApplication.visaRejectedReason || '',
//             VisaRejectedCountry: this.stuApplication.visaRejectedCountry || '',
//             EnglishTestType: this.stuApplication.englishTestType || 'Applied', 
...
//   nextStep() {
//     if (this.isEditingStep[this.currentStep]) {
//       Swal.fire('Please Update or Cancel', 'Save or cancel current changes before moving to next step.', 'warning');
//       return;
//     }
...
//     const consentLetterUploaded = !!this.ConsentLetterFileName || this.stuApplication.consentLetterFileName.length != 0;

//     return feesProofUploaded && resumeUploaded && passportUploaded && consentLetterUploaded;
//   }
...
// //   // ── Employee info ────────────────────────────────────────────────────────────
// //   DealingFacultyName: any; CounsellingAuthorityName: any;
...
// //   const sponsorType =
// //     this.form.get('sponsorType')?.value;

// //   return ['Parent', 'Parents']
// //     .includes((sponsorType || '').trim());
// // }
// //   // isCurrentStepValid(step: number): boolean {
// //   //   if (!this.form) return false;
...
// //       this.form.get('sponsorName')?.setValidators([
// //         Validators.required
// //       ]);

// //       this.form.get('sponsorRelation')?.setValidators([
// //         Validators.required
// //       ]);
// //     }
...
// //     Swal.fire(
// //       'Please Update or Cancel',
// //       'Save or cancel current changes before moving to next step.',
// //       'warning'
// //     );
...
// //     Swal.fire(
// //       'Please Update or Cancel',
// //       'You are currently in edit mode. Please click "Update" to save your changes or "Cancel" to discard them before moving back.',
// //       'warning'
// //     );
// //     return;
// //   }
